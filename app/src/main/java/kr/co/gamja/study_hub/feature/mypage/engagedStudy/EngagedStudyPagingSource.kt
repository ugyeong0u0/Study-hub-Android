...
class EngagedStudyPagingSource(private val studyHubApi: StudyHubApi) :
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, ContentX> {
        return try {
            ...
            val response = studyHubApi.participatingMyStudy(currentPageNumber, params.loadSize)
            val responseData = response.body()?.participateStudyData?.content ?: emptyList()

            val nextPageNumber =
                if (response.body()?.participateStudyData?.last ==true) {
                    null
                } else {
                    currentPageNumber + 1
                }
